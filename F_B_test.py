...
file_path = '/data/user/runzeli/July/test_100B'
#file_path = '.'
...
def getB(x, y, z, a):
    ...
    return np.array([B_x, B_y, B_z]) * pow(10, -9) * 100
...
class muon(object):
    ...
    def get_dis_need(self, E):
        ...
        density = 0.9167
        temp = (E * b + a) / (E * b * self.updateref + a)

        dis = 1/(density * b) * log(temp)
    ...
    def updatepos(self, t, p, a):
        ...
        const_B = getB(self.pos[0], self.pos[1], self.pos[2], a)
        ...
        const_B_new = getB(pos_new[0], pos_new[1], pos_new[2], a)     # new B field after this time period todo: also no -1
        ...
        if Dis > 10000:
            ...
            exit(1)
        """
        dis_need = self.get_dis_need(self.E)
        timet = dis_need/self.v_total
...
def MC_simulation(theta_min, theta_max, E_min, E_max, num, index, pos_index):

    if not os.path.exists(file_path + "/data"):
      os.makedirs(file_path + "/data")
    
    if not os.path.exists(file_path + "/data/" + str(pos_index)):
      os.makedirs(file_path + "/data/" + str(pos_index))

    a = GeoMag()

    final_t_list = []
    final_p_list = []
    final_solid_ang_list = []
    final_integral_list = []
    final_base_list = []
    final_E_list = []
    initial_E_list = []
    for i in range(num):
        ...
        E, zenith, azimuth, solid_ang, integral, base = get_rand_ini(icecube_theta, E_min, E_max)
        ...
        #writer = open(file_path + "/data" + "/" + str(index) + ".txt", 'w')

        check, final_dir, final_E = main(v_dir, p, E, pos, a)

        print(i)
        print(degrees(icecube_theta), degrees(phi), E)

        if check == 0:

            final_theta, final_phi = convert_final_dir(final_dir)

            final_t_list.append(final_theta)
            final_p_list.append(final_phi)
            final_solid_ang_list.append(solid_ang)
            final_integral_list.append(integral)
            final_E_list.append(final_E)          
            initial_E_list.append(E)
            final_base_list.append(base)
            print(str(degrees(final_theta)) + " " + str(degrees(final_phi)) + " " + str(solid_ang) + " " + str(integral) + " " + str(base) + " " + str(E))
            
        print("check is " + str(check))
        print()
    writer = open(file_path + "/data/" + str(pos_index) + "/" + str(index) + ".txt", 'w')

    for i in range(len(final_t_list)):
        writer.write(str(final_t_list[i]) + " " + str(final_p_list[i]) + " " + str(final_solid_ang_list[i]) + " " + str(final_integral_list[i]) + " " + str(final_base_list[i]) + " " + str(final_E_list[i]) + " " + str(initial_E_list[i]) + "\n")
...
parser.add_argument('pos_index', metavar='N', type=int, nargs='+', help='index from 0 to 4')


args = parser.parse_args()
run_index = int(args.run_index[0])
pos_index = int(args.pos_index[0])
...
MC_simulation(theta_min, theta_max, E_min, E_max, num, run_index, pos_index)
